...
function setFixedPosition() {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/firebase-messaging-sw.js')
      .then(function(registration) {
        ...
        messaging.getToken({
          vapidKey: "BBFMxK7zB18uRRRKmN1rBiMY1pvuIL3dfw46U1jmZsHl4cMMQXr_yhyQ2ElGjHaV-qSYHAv_dgShXrvlM1Qayss", // ganti dengan VAPID kamu dari Firebase
          serviceWorkerRegistration: registration
        })
        .then((currentToken) => {
          if (currentToken) {
            // Kirim ke Google Apps Script
            fetch("https://script.google.com/macros/s/AKfycbxx_LoHKA83PclVfYuCQ5AhRYLCRIcESEBYV6YsiTQiA1BZ8KcgsTrnCEq_w-zr2IPl/exec", {
              method: "POST",
              headers: {
            ...
            .then(res => res.text())
            .then(result => {
              console.log("Token berhasil dikirim:", result);
              document.getElementById("notif-status").textContent = "✅ Notifikasi diaktifkan.";
              document.getElementById("notif-status").style.color = "green";
            })
            .catch(err => {
              console.error("Gagal kirim token:", err);
              document.getElementById("notif-status").textContent = "❌ Gagal mengirim token.";
              document.getElementById("notif-status").style.color = "red";
            });
          } else {
            console.warn("Token tidak tersedia.");
            document.getElementById("notif-status").textContent = "❌ Token tidak tersedia. Pastikan Anda memberi izin notifikasi.";
            document.getElementById("notif-status").style.color = "red";
          }
        })
        .catch(err => {
          console.error("Gagal ambil token:", err);
          document.getElementById("notif-status").textContent = "❌ Gagal mengambil token.";
          document.getElementById("notif-status").style.color = "red";
        });
      });
  }
  ...
  messaging.getToken({
    vapidKey: "BBFMxK7zB18uRRRKmN1rBiMY1pvuIL3dfw46U1jmZsHl4cMMQXr_yhyQ2ElGjHaV-qSYHAv_dgShXrvlM1Qayss"
  })
  .then((currentToken) => {
    if (currentToken) {
